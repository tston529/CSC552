...
<html>
    <head>
        ...
        <title>States' Health Stats</title>
    </head>
    <body>
        ...
        <div Align=Center>
            <form name='loginForm' method='post' action='${pageContext.request.contextPath}/HealthDBAccess'>
                ...
                <select name='topic' required='true'>
                    <option value='(Variable Topic)' selected disabled>(SELECT VARIABLE TOPIC)</option>
        ...
        <c:forEach items="${errorMsgs}" var="err">
            ${err}
        </c:forEach>
        ...
        <div Align=Center>
        <table>
            <c:forEach items="${healthRow}" var="row">
                <tr>
                    <td>${row.location}</td>
                    <td>${row.value}</td>
                </tr>
